...
_direct_conn_establish(pmixp_conn_t *conn, void *_hdr, void *msg)
{
	...
	if (hdr->ext_flag) {
		...
		if (rc) {
			free_buf(buf_msg);
			close(fd);
			PMIXP_ERROR("Failed to unpack the direct connection message from %u",
				    hdr->nodeid);
			return;
		}
		...
		if (rc) {
			free_buf(buf_msg);
			close(fd);
			PMIXP_ERROR("Connection reject from %u", hdr->nodeid);
			return;
		}
		nodename = pmixp_info_job_host(hdr->nodeid);
		PMIXP_DEBUG("Authentication successful from %s", nodename);
		xfree(nodename);
		free_buf(buf_msg);
	} else {
		PMIXP_ERROR("Connection failed from %d", hdr->nodeid);
		close(fd);
		return;
	}
