...
#include "src/common/base64_encode.h"
                         ...
                         pmix_op_cbfunc_t cbfunc, void *cbdata)
{
	...
	caddy->base64_setup_str =
	                base64_encode((char *)pbo.bytes, pbo.size, &len);
...
{
	char *p = getenvp(*env, PMIXP_SERVER_SETUP_APP);
	pmix_info_t *info;
	int32_t ninfo, cnt;
	size_t len;
	...
	buf = base64_decode(p, strlen(p), &len);
                                    ...
                                    char *mapping, char ***env)
{
	...
	PMIXP_DEBUG("packed test struct: %p", setup_app_caddy->base64_setup_str);
	if (setup_app_caddy->base64_setup_str) {
		setenvf(env, PMIXP_SERVER_SETUP_APP, "%s",
		        setup_app_caddy->base64_setup_str);
		PMIXP_DEBUG("packed test struct: %s", setup_app_caddy->base64_setup_str);
	}
